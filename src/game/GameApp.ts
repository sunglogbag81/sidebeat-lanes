...
export class GameApp {
  ...
  private async toggleStart(): Promise<void> {
    ...
    if (!this.state.startedAt || this.state.finished) { if (this.state.finished) this.reset(); this.state.startedAt = now; this.state.running = true; if (this.audio.src) { this.audio.currentTime = 0; await this.audio.play(); } this.lastFrame = now; this.animationId = requestAnimationFrame((t) => this.loop(t)); return; }
    this.state.running = !this.state.running;
    if (this.state.running) { this.state.pauseAccumulated += now - this.state.pausedAt; if (this.audio.src) await this.audio.play(); this.lastFrame = now; this.animationId = requestAnimationFrame((t) => this.loop(t)); }
    else { this.state.pausedAt = now; this.audio.pause(); cancelAnimationFrame(this.animationId); this.draw(this.songTime()); this.setStatus('일시정지됨.'); }
  }
  ...
  private finish(): void { this.state.running = false; this.state.finished = true; this.audio.pause(); cancelAnimationFrame(this.animationId); const acc = this.accuracy(); must<HTMLElement>('#resultRank').textContent = rankForAccuracy(acc); must<HTMLElement>('#resultScore').textContent = Math.round(this.state.score).toLocaleString('ko-KR'); must<HTMLElement>('#resultCombo').textContent = String(this.state.maxCombo); must<HTMLElement>('#resultAccuracy').textContent = `${acc.toFixed(2)}%`; must<HTMLElement>('#resultNotes').textContent = String(this.state.notes.length); must<HTMLDialogElement>('#resultDialog').showModal(); }
